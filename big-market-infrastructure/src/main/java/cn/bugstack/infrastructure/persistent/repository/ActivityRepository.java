...
public class ActivityRepository implements IActivityRepository {
    ...
    public void doSaveOrder(CreateQuotaOrderAggregate createOrderAggregate) {
        ...
        try{
            ...
            // 以用户ID作为切分键，通过doRouter设定路由[这样就保证了下面的操作，都是同-个链接下，也就保证了事务的特性]
        ...
        try{
            ...
            transactionTemplate.execute(status -> {
                try {
                    ...
                    // 2.创建或更新月账户，true - 存在更新，false - 不存在则插入
                    ...
                    // 3.创建或更新日账户，true - 存在更新，false - 不存在则插入
                    ...
                    status.setRollbackOnly();
                    log.error("写入创建参与活动，唯一索引冲突 userId:{} activityId:{}",userId,activityId,e);
